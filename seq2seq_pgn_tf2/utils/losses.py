...
def pgn_log_loss_function(real, final_dists, padding_mask):
    ...
    batch_nums = tf.range(0, limit=real.shape[0])  # shape (batch_size) [0,1,2,3]
    for dec_step, dist in enumerate(final_dists):
        # The indices of the target words. shape (batch_size)
        targets = real[:, dec_step] # [200,301,203,4023]
        indices = tf.stack((batch_nums, targets), axis=1)  # shape (batch_size, 2) [[0,200],[1,301],[2,203],[3,4023]]
        # gold_probs = []
...
if __name__ == '__main__':
    a = tf.range(0, 2, dtype=tf.float32)
    print(a)
    b = tf.constant([1, 3])
    print(b)
