...
class BahdanauAttentionCoverage(tf.keras.layers.Layer):
    ...
    def call(self, dec_hidden, enc_output, enc_padding_mask, use_coverage=False, prev_coverage=None):
        """
        :param dec_hidden: shape=(32, 256)
        :param enc_output: shape=(32, 200, 256)
        :param enc_padding_mask: shape=(32, 200)
        :param use_coverage: default False, if True, use coverage
        ...
        hidden_with_time_axis = tf.expand_dims(dec_hidden, 1)  # shape=(32, 1, 256)
        # att_features = self.W1(enc_output) + self.W2(hidden_with_time_axis)

        def masked_attention(score):
            """
            :param score: shape=(32, 200, 1)
                        ...
              [-0.50474256]
              [-0.47997713]
              [-0.42284346]]]
            :return:
            """
            attn_dist = tf.squeeze(score, axis=2)  # shape=(32, 200)
            attn_dist = tf.nn.softmax(attn_dist, axis=1)  # shape=(32, 200) 200个词的注意力分布
        ...
        if use_coverage and prev_coverage is not None:  # non-first step of coverage
            ...
            score = self.V(tf.nn.tanh(self.W1(enc_output) + self.W2(hidden_with_time_axis) + self.Wc(prev_coverage)))
            # Calculate attention distribution
            attn_dist = masked_attention(score)
            # Update coverage vector
            coverage = attn_dist + prev_coverage

        else: # normal attention or prev_coverage is None
            # Calculate e = v^T tanh(W_h h_i + W_s s_t + b_attn)
            score = self.V(tf.nn.tanh(self.W1(enc_output) + self.W2(hidden_with_time_axis)))  # shape=(32, 200, 1)
            # Calculate attention distribution
            attn_dist = masked_attention(score)  # shape=(32, 200, 1)
        ...
        context_vector = attn_dist * enc_output  # shape=(32, 200, 256)
        context_vector = tf.reduce_sum(context_vector, axis=1)  # shape=(32, 256)
...
class Pointer(tf.keras.layers.Layer):
    ...
    def call(self, context_vector, state, dec_inp):
        p_gen = tf.nn.sigmoid(self.w_c_reduce(context_vector)+self.w_s_reduce(state)+self.w_i_reduce(dec_inp))
        return p_gen